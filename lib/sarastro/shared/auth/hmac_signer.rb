#
# Adaptation from the code authored by:
# https://github.com/Asquera/warden-hmac-authentication/blob/master/lib/hmac/signer.rb
#
  ...
  module HMAC
    ...
    class Signer
      ...
      def sign_request(url, secret, opts = {})
        ...
        date = opts[:date] || Time.now.utc
        date = date.utc.strftime('%a, %e %b %Y %H:%M:%S UTC') if date.respond_to? :strftime
      ...
      def self.nonce(size = 32)
        (0...size).map{ (('A'..'Z').to_a + ('a'..'z').to_a + ('0'..'9').to_a)[Random.rand(62)] }.join
      end
    end
  end
end